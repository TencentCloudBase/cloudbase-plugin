// hooks/logger.mjs — CloudBase plugin logger (adapted from Vercel plugin)
import { randomBytes } from "crypto";

var LEVELS = ["off", "summary", "debug", "trace"];
var LEVEL_INDEX = { off: 0, summary: 1, debug: 2, trace: 3 };
var SHARED_LOGGER_CONTEXT_KEY = "__cloudbasePluginSharedLoggerContext__";
...
function resolveLogLevel() {
  const explicit = (process.env.CLOUDBASE_PLUGIN_LOG_LEVEL || "").toLowerCase().trim();
  if (explicit && LEVEL_INDEX[explicit] !== void 0) return explicit;
  if (explicit) {
    console.error(
      `[cloudbase-plugin] Unknown CLOUDBASE_PLUGIN_LOG_LEVEL="${explicit}". Valid levels: ${LEVELS.join(", ")}. Falling back to "off".`
    );
  }
  if (process.env.CLOUDBASE_PLUGIN_DEBUG === "1" || process.env.CLOUDBASE_PLUGIN_HOOK_DEBUG === "1") {
    return "debug";
  }
  return "off";
}
...
function getSharedLoggerContext() {
  ...
  if (!loggerGlobal[SHARED_LOGGER_CONTEXT_KEY]) {
    loggerGlobal[SHARED_LOGGER_CONTEXT_KEY] = {};
  }
  return loggerGlobal[SHARED_LOGGER_CONTEXT_KEY];
}
...
export { LEVELS, LEVEL_INDEX, createLogger, logCaughtError, logDecision, resolveLogLevel, serializeErrorForLog };
