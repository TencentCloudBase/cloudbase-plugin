// hooks/hook-env.mjs — CloudBase plugin environment utilities (adapted from Vercel plugin)
...
function resolveAuditLogPath(hookInputCwd) {
  ...
  const configuredPath = process.env.CLOUDBASE_PLUGIN_AUDIT_LOG_FILE;
  if (configuredPath === "off") return null;
...
function dedupScopeIdSegment(scopeId) {
  if (SAFE_SESSION_ID_RE.test(scopeId)) return scopeId;
...
function resolveDedupTempPath(sessionId, basename, scopeId) {
  ...
  const candidate = resolve(join(tempRoot, `cloudbase-plugin-${dedupSessionIdSegment(sessionId)}${scopeSegment}-${basename}`));
...
var CLEARABLE_SESSION_KINDS = new Set(["seen-skills", "seen-context-chunks"]);

function removeAllSessionDedupArtifacts(sessionId) {
  const result = { removedFiles: 0, removedDirs: 0 };
  const tempRoot = resolve(tmpdir());
  const prefix = `cloudbase-plugin-${dedupSessionIdSegment(sessionId)}-`;
  let entries;
  try {
    entries = readdirSync(tempRoot).filter((name) => {
      if (!name.startsWith(prefix)) return false;
      for (const kind of CLEARABLE_SESSION_KINDS) {
        if (name.endsWith(`-${kind}.d`) || name.endsWith(`-${kind}.txt`)) return true;
      }
      return false;
    });
  } catch {
